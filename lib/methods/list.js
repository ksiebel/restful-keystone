...
module.exports = function (list, config, entry) {
  config = _.defaults({
    name: list.path
  }, config);
  return {
    handle: function (req, res, next) {
      ...
      var resultsPerPage = Number(options.perPage) || 30;
      ...
      query.exec = function (next) {
        query.count(function (err, count) {
          ...
          query.find().limit(resultsPerPage).skip(skipping);
